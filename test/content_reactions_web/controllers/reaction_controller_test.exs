...
#  use ExMachina
  ...
  describe "create reaction" do
    test "create reaction with valid params", %{conn: conn} do
      ...
      qouted = quote do: :ets.lookup(:reactions, {unquote(reaction.content_id), unquote(reaction.user_id), unquote(reaction.reaction_type)}) |> Enum.empty?
      assert eventually(false, qouted)


      assert {{reaction.content_id, reaction.user_id, reaction.reaction_type}, 1} in :ets.lookup(:reactions, {reaction.content_id, reaction.user_id, reaction.reaction_type})
    end
  ...
  describe "read reactions" do
    ...
    test "it does not increment counter for redundant request", %{conn: conn} do
      ...
      conn = get(conn, Routes.reaction_path(conn, :reaction_counts, reaction.content_id))
      assert json_response(conn, 200) == %{"content_id" => reaction.content_id, "reaction_count" => %{"Fire" => 1}}
    ...
    test "it decrements counter for `remove` action", %{conn: conn} do
      ...
      conn = get(conn, Routes.reaction_path(conn, :reaction_counts, reaction.content_id))
      assert json_response(conn, 200) == %{"content_id" => reaction.content_id, "reaction_count" => %{"Fire" => 0}}
    end

    test "it decrements counter for `remove` action only if a user added a reaction before", %{conn: conn} do
      user1_reaction = build_reaction(%{reaction_type: 1})
      user2_reaction_remove = build_reaction(%{content_id: user1_reaction.content_id, user_id: Ecto.UUID.generate(),reaction_type: 1, action: 0})
      ...
      assert json_response(conn, 200) == %{"content_id" => user1_reaction.content_id, "reaction_count" => %{"Fire" => 1}}


      post(conn, Routes.reaction_path(conn, :create), reaction: user2_reaction_remove)
      conn = get(conn, Routes.reaction_path(conn, :reaction_counts, user2_reaction_remove.content_id))
      assert json_response(conn, 200) == %{"content_id" => user2_reaction_remove.content_id, "reaction_count" => %{"Fire" => 1}}
    end
